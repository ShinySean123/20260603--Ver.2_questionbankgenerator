...
st.markdown("共筆組長專專屬完全體：結合【講義智慧出題】、【純題配詳解】與【現成題庫轉 Word】三大核心功能！")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("💡 頻率守護防線：系統已啟動【5 RPM 智慧速率緩衝調節】。若間隔安全則 0 秒全速直通；若偵測到連點或多人共用，後台將自動微秒倒數，穩卡安全限制。")
...
# ==================== 🌟 [速率導航升級] 共享的 5 RPM 智能防禦函數 ====================
def generate_content_via_http_with_retry(contents_list, api_key, max_retries=4):
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={api_key}"
    
    # 🌟 核心步驟一：精算 5 RPM（每分鐘 5 次 = 每 12 秒點擊 1 次算安全線）
    current_time = time.time()
    elapsed_since_last_call = current_time - st.session_state["last_api_call_time"]
    
    REQUIRED_GAP = 12.5 # 設定 12.5 秒為完美安全間隔，多保留 0.5 秒防網速波動
    
    if elapsed_since_last_call < REQUIRED_GAP:
        # 計算出最精準、需要原地等待的秒數
        needed_sleep_seconds = REQUIRED_GAP - elapsed_since_last_call
        st.warning(f"🛡️ 觸發 5 RPM 速率防禦限制！為了防止 429 限流報錯，後台正在智慧導航緩衝，請稍候 {needed_sleep_seconds:.1f} 秒後自動直通發送...")
        time.sleep(needed_sleep_seconds)
    ...
    for attempt in range(max_retries):
        ...
        # 🌟 核心步驟二：只要連線一建立成功，立刻向全域刷新最新的敲門時間戳記
        ...
        if st.button("⚡ 開始全全自動雙模融合出題 ⚡", use_container_width=True):
            try:
                ...
                with st.spinner("🧠 文字包封裝完成！AI 正在進行歷史考點深層去重並出題中..."):
                    ...
                    【詳解與出處恆定要求】：
                    - 不論前面題目成中文還是英文，【針對各選項之詳解】必須一律使用繁體中文進行極為詳細的專家級辨析。
