...
if "模組 A" in main_mode:
    ...
    sub_function_mode = st.radio(
        "🔥 請選擇您的智慧出題執行方式：",
        ["🚀 功能 A'：客製化生成專屬 PROMPT ➡️ 自己複製拿去餵給 NotebookLM / 任意大模型 AI", "✅ 功能 A：直接點擊 ➡️ 網頁自動呼叫雲端 AI 一鍵吐出檔案"],
        index=0,
        horizontal=True
    )
    st.markdown("---")
    ...
    if "功能 A'" in sub_function_mode:
        ...
        with col_opt2:
            ai_target = st.radio(
                "🤖 請選擇預計使用的目標 AI 平台：",
                ["1. NotebookLM (預設：依據講義上印製的頁碼標註)", "2. Gemini / ChatGPT (依據講義上印製的頁碼標註)"],
                index=0,
                key="ap_ai_target"
            )
...
"""
            ...
            source_prompt_str = """
【📌 出處與頁碼精準度鐵律 - 依據講義頁面印製頁碼】：
- 【出處】必須精準對照來源檔案，標註出處來源 PDF 檔案名稱與講義內印製的實際頁碼。
- 標註格式統一為：=== 【檔名.pdf】第 X 頁 ===
- 若題幹或選項涵蓋/引用了講義中的多個頁面，請務必將所有相關頁碼一併附註列出（例如：=== 【檔名.pdf】第 1、3、5、6 頁 ===）。
- 請直接偵測與採用講義 PDF 頁面或投影片上『實際印製標註的頁碼數字』作為頁數依據。
"""
            if a_prime_dedup:
                history_prompt_str = """
【🚨 歷史考點與舊題去重指令】：
...
【針對各選項之詳解深度與品質鐵律 - 專家級逐項剖析】：
- 不論前面題目是中文還是英文，【針對各選項之詳解】必須一律使用繁體中文進行極為詳細的專家級辨析（換行請用 \\n 符號）。
            ...
            if st.button("⚡ 開始全全自動雙模融合出題 ⚡", use_container_width=True):
                ...
                try:
                    ...
                    with st.spinner("🧠 任務封裝完成！正發送至雲端核心進行智慧出題..."):
                        ...
                        【📌 出處與頁碼精準度鐵律 - 依據講義頁面印製頁碼】：
                        - 【出處】請精準對照我文本中的頁碼標籤（例如：=== 【檔名.pdf】第 X 頁 ===），指出這題是出自哪一個檔案的第幾頁！
                        - 若題幹或選項涵蓋/引用了講義中的多個頁面，請將所有相關頁碼一併附註列出（例如：=== 【檔名.pdf】第 1、3、5、6 頁 ===）。
                        - 請直接偵測並讀取講義頁面/投影片上『實際印製標註的頁碼數字』進行引用！
                        - 正確答案（A, B, C, D, E）的總體數量分布要稍微平均一些。
